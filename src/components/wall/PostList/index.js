...
class PostList extends Component {
  ...
  filterAdList = async (hashtags) => {
    ...
    this.props.adListNoAuth({hashtags:hashtags});
  ...
  onPageChange = page => {
    ...
    if(this.props.token)
    this.props.adList({hashtags:this.state.hashtags,page:page});
    else
    this.props.adListNoAuth({hashtags:this.state.hashtags,page:page});
  ...
  generateAdItem = () =>{
    ...
    let AdItems = this.props.adsList.details.map((post) => {
      ++k;
  ...
  generateNoAuthAdItem = () =>{
    let k=0
    if(this.props.adNoAuthList.details.length == 0){
      return <Empty/>;
    }
    let AdItems = this.props.adNoAuthList.details.map((post) => {
      ++k;
      console.log("<-------"+k+"---post---------------->",post)
      return <AdItem key={k} post={post} user={this.props.userDetails} />
    })
  ...
  render() {
    ...
    return (
      <Auxiliary>
        ...
        <Pagination current={this.state.current} onChange={this.onPageChange} total={this.props.adsList.total} defaultPageSize={10} defaultCurrent={1}/>
        {this.generateAdItem()}
        <Pagination current={this.state.current} onChange={this.onPageChange} total={this.props.adsList.total} defaultPageSize={10} defaultCurrent={1}/>
      ...
      return (
        <Auxiliary>
          <FilterBar filterAdList={this.filterAdList.bind(this)} hashtags={this.props.hashtagsList} hashtagList={this.props.hashtagList}  placeholder="Filter By Hashtags" style={{ width: "75%", margin:"5px" }}  />
          { this.props.adNoAuthList.details.length > 0 &&
            <Pagination current={this.state.current} onChange={this.onPageChange} total={this.props.adNoAuthList.total} defaultPageSize={10} defaultCurrent={1}/>
            }
